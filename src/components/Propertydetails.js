...
function PropertyDetails() {
  const { id } = useParams(); 
  const property = hotelsData.find((hotel) => hotel.id === parseInt(id, 10));
  ...
  return (
    ...
    <div className=" flex flex-col  justify-evenly bg-white border rounded-2xl p-4 m-2 ">
    ...
    <div className="relative">
      ...
      />
      <div className=" text-blue-700 rounded-2xl  absolute top-3 left-3 bg-white p-2 font-semibold tracking-wider cursor-pointer hover:bg-slate-200 ">
        For Rent
      </div>
    ...
    <div className="flex justify-between items-center m-2">
      <p>
        <span className="text-blue-700 font-bold text-2xl">${property.price}</span>
        <sub className=" text-xl">/month</sub>
      </p>
      <Link
        to="/"
        className="bg-blue-700 text-white px-4 py-2 mt-2 inline-block rounded-2xl text-center m-2"
      >
        Back to Listings
      </Link>
    </div>
